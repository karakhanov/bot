...
from main.models import temp, userTelegram
...
class Command(BotBase):
    def delete(self, update: Update, context: CallbackContext)-> None:
        try:
            ...
            user = userTelegram(id_telegram=update.effective_user.id, bed=1, baby=0, pet=0, breakfast=0)
            user.save()
        if user.baby == '0':
            user.baby = "1"
        elif user.baby == '1':
    ...
    def start(self, update: Update, context: CallbackContext) -> None:
        try:
            ...
            user = userTelegram(id_telegram=update.effective_user.id, bed=1, baby=0, pet=0, breakfast=0)
            user.save()
        # url = 'https://dev.kwhotel.com/api/v2/auth'
        # data = {
        # "surname": "ali",
        # "phone": "+998999999999",
    ...
    def days(self, update: Update, context: CallbackContext)-> None:
        ...
        while i <= 5:
            while y <= 6:
                tmp.append(InlineKeyboardButton('{0:02d}.{1:02d}'.format(asd.day,asd.month), callback_data='{0:02d}.{1:02d}'.format(asd.day,asd.month)))
